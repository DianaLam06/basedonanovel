...
read_books <- function(i){
  ...
  books <- temp %>% mutate(
   ...
   ( str_trim(str_to_lower(cleanTitle)) %in% str_trim(str_to_lower(imdb_books$TitleBook))) |
   ( str_trim(str_to_lower(cleanTitle)) %in% str_c(imdb_books$simpleTitleBook," : a novel")) |
     ( str_trim(str_to_lower(cleanTitle)) %in% str_c("the ", imdb_books$simpleTitleBook)) ,
...
write_csv(full, "./data/CSV/checkedout_more_titles.csv")
